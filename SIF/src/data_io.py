...
def prepare_data(list_of_seqs, ids):
    lengths = []
    partial_len = []
    for s in list_of_seqs:
        lengths.append(len(s))
        for c in s:
            partial_len.append(len(c))
        
    #lengths = [len(s) for s in list_of_seqs]
    n_samples = len(list_of_seqs)
    maxlen = np.max(lengths)
    partial_maxlen = np.max(partial_len)
    x = np.zeros((n_samples, maxlen, partial_maxlen)).astype('int32')
    x_mask = np.zeros((n_samples, maxlen, partial_maxlen)).astype('float32')
    x_loc = np.zeros((n_samples, maxlen, partial_maxlen)).astype('int32')

    t=0

    for idx, s in enumerate(list_of_seqs):
        for idy, c in enumerate(s):
            x_mask[idx, idy, :partial_len[t]] = 1.
            x[idx, idy, :partial_len[t]] = c

            id_loc = ids[idx, idy]
            id_len = id_loc[1] - id_loc[0] + 1
            id_list = list(range(id_loc[0]+1, id_loc[1]+2))
            '''
            print('------- id_list --------')
            print(id_list)
            ...
            print('1 : ', id_loc[1])
            '''
            x_loc[idx, idy, :id_len] = id_list

            t += 1
        #x_mask[idx, :lengths[idx]] = 1.
    x_mask = np.asarray(x_mask, dtype='float32')
    return x_loc, x, x_mask
...
def sentences2idx(seq_loc, sentence_ids):
    """
    ...
    """
    seq1 = []
    for i in range(len(seq_loc)):
        seq1.append(getSeq(seq_loc[i], sentence_ids[i]))
    x_loc, x1, m1 = prepare_data(seq1, seq_loc)
    return x_loc, x1, m1
    ...
    for i in lines:
        ...
        if(len(i) > 0):
            ...
            if(len(i) == 2):
                if i[0] in unks:
                    #print(i[0])
                    #word2weight['<unk>'] += float(i[1])
                    continue
                ids = tokenizer.convert_tokens_to_ids(i[0]) #ids是BERT id
                word2weight[ids[0]] = float(i[1])
    ...
    for i in range(seq.shape[0]):
        for j in range(seq.shape[1]):
            for k in range(seq.shape[2]):
                if mask[i,j,k] > 0 and seq[i,j,k] >= 0:
                    if seq[i,j,k] not in token_ids :
                        try:
                            weight[i,j,k] = weight4ind[seq[i,j,k]]
                        except:
                            weight[i,j,k] = 1
        '''
        print('------------ seq : ', i, ' -----------')
